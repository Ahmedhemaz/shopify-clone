...
export class Password implements IValueObject<Password> {
  ...
  constructor(password: string) {
    if (isEmpty(password, { ignore_whitespace: true }))
      throw new EmptyStringException(ErrorMessages.EMPTY_PASSWORD);
    if (!isLength(password, { min: 8 }))
      throw new StringLengthException(ErrorMessages.PASSWORD_LENGTH);
    if (!this.isValidPasswordFormat(password))
      throw new PasswordFormatException(ErrorMessages.INVALID_PASSWORD_FORMAT);
  ...
  private isValidPasswordFormat(password: string): boolean {
    return password.match(/^(?=.{8,})(?=.*[a-z])(?=.*[A-Z])(?=.*[@#$%^&+=]).*$/)
      ? true
      : false;
  }
}
